"""Connection lifecycle for Finout Connector."""
...
@chat.function(
    "connect_finout",
    "Connect Finout account via credentials.",
    action_type="write",
    chain_callable=True,
    event="finout-connector.connect_finout",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_finout(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Finout Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Finout Account",
...
@chat.function(
    "list_connections",
    "List connected Finout accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_finout",
    "Disconnect Finout account.",
    action_type="write",
    chain_callable=True,
    event="finout-connector.disconnect_finout",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_finout(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
